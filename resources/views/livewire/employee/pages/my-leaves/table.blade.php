							...
							@if (count($leaves) > 0)
								@foreach ($leaves as $leave)
									<tr class="hover:bg-slate-100">
										...
										<td class="px-5 py-2 border-b border-gray-200 text-sm">
											<p class="text-gray-900 whitespace-no-wrap">{{ $leave->leave_from }}</p>
										</td>
										<td class="px-5 py-2 border-b border-gray-200 text-sm">
											<p class="text-gray-900 whitespace-no-wrap">{{ $leave->leave_to }}</p>
										</td>
										<td class="px-5 py-2 border-b border-gray-200 text-sm">
											<p class="text-gray-900 whitespace-no-wrap">
												@if ($leave->leave_status == 'CONFIRMED')
													<span class="bg-green-100 text-green-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-green-200 dark:text-green-900">{{ $leave->leave_status }}</span>
												@endif
												@if ($leave->leave_status == 'PENDING')
													<span class="bg-red-100 text-red-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-red-200 dark:text-red-900">{{ $leave->leave_status }}</span>
												@endif
